...
public class OrderDao {
    ...
    public void add(Order order) throws OrderSystemException {
        ...
        addOrderUser(order);
        //2.再操作order_dish表
        //执行add方法的时候，order对象中的orderId字段还是空着的
        //这个字段要交给数据库，由自增主键来决定
        addOrderDish(order);
    }
    ...
    private void addOrderDish(Order order) throws OrderSystemException {
        ...
        try {
            ...
            //6.发送给服务器(真的执行),commit可以去执行多个SQL,一次调用commit统一发给服务器
            connection.commit();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public List<Order> selectAll() {
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                order.setIdDone(resultSet.getInt("isDone"));
                orders.add(order);
            }
        } catch (SQLException e) {
            e.printStackTrace();
